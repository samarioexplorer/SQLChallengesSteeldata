...
INSERT INTO Customersofbank (CustomerID, FirstName, LastName, City, State)
...
FOREIGN KEY (CustomerID) REFERENCES Customersofbank(CustomerID),
...
FROM customersofbank
WHERE City = 'New York'
ORDER BY LastName ASC;

#2. What is the total number of accounts in the Accounts table?
SELECT COUNT(AccountID) as Accountsregistered 
FROM accounts;
...
SELECT SUM(Balance) AS TotalBalance
FROM accounts ac
INNER JOIN customersofbank cb ON ac.CustomerID = cb.CustomerID
WHERE cb.city = 'Los Angeles';

#5. Which branch has the highest average account balance?
SELECT br.BranchName, ROUND(AVG(Balance)) AS HighestAvgAccountBalance
FROM accounts ac
INNER JOIN branches br ON ac.BranchID = br.BranchID
GROUP BY br.BranchName
ORDER BY ROUND(AVG(Balance)) DESC
LIMIT 1;

#6. Which customer has the highest current balance in their accounts?
SELECT CONCAT( FirstName, " ", LastName ) as Customer, ROUND(SUM(Balance)) AS HighestCurrentBalance
FROM accounts ac
INNER JOIN customersofbank cb ON ac.CustomerID = cb.CustomerID
WHERE ac.AccountType = 'Checking'
GROUP BY CONCAT( FirstName, " ", LastName )
ORDER BY SUM(Balance) DESC
LIMIT 1;

#7. Which customer has made the most transactions in the Transactions table?
SELECT CONCAT( FirstName, " ", LastName ) as Customer, COUNT(tr.TransactionID) AS Transactions
FROM accounts ac
JOIN customersofbank cu
...
SELECT BranchName, ROUND(SUM(ac.Balance)) AS TotalBalance
FROM branches br 
JOIN accounts ac 
ON br.BranchID = ac.BranchID
GROUP BY BranchName
ORDER BY ROUND(SUM(ac.Balance)) DESC
LIMIT 1;
...
SELECT CONCAT( FirstName, " ", LastName ) as Customer, ROUND(SUM(Balance)) AS HighestCurrentBalance
FROM accounts ac
JOIN customersofbank cb ON ac.CustomerID = cb.CustomerID
WHERE ac.AccountType in ("Savings", "Checking")
GROUP BY CONCAT( FirstName, " ", LastName )
ORDER BY SUM(Balance) DESC
LIMIT 1;

#10. Which branch has the highest number of transactions in the Transactions table?
SELECT BranchName, Count(TransactionID) AS Transactions
